...
fn main() -> Result<(), String> {
    ...
    ValueIterator::new(chars)
        ...
        .try_for_each(|caso_teste| -> Result<(), String> {
            let num = caso_teste[0].as_int().ok_or(format!("Invalid input number: {:?}", caso_teste[0]))?;
            let expected = caso_teste[1].as_string().ok_or_else(|| format!("Expected string value, got {:?}", caso_teste[1]))?;
