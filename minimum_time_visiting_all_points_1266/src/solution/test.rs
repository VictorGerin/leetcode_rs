#[cfg(test)]

use super::solution::*;

#[test]
fn minimum_time_visiting_all_points_1266() -> Result<(), String> {
    use leetcode_lib::parser::{Val, ValIter, ProcessInputError, read_input};
    
    ValIter::new(read_input("input.txt")?)
        .collect::<Result<Vec<Val>, ProcessInputError>>()
        .map_err(|e| e.to_string())?
        .chunks_exact(2)
        .try_for_each(|caso_teste| -> Result<(), String> {
            let points = caso_teste[0].clone()
                .as_vec()
                .map_err(|x| format!("Expected array, got {:?}", x))?
                .into_iter()
                .map(|x| -> Result<Vec<i32>, String> {
                    Ok(x.as_vec_int()
                    .map_err(|x| format!("as_vec_int: {:?}", x))?)
                }).collect::<Result<Vec<Vec<i32>>, String>>()?;

            let expected = caso_teste[1].clone()
                .as_int()
                .map_err(|x| format!("Expected integer, got {:?}", x))?;

            let result = Solution::min_time_to_visit_all_points(points.clone());
            assert_eq!(result, expected, "Expected result to be {}, but got {} for input {:?}", expected, result, points);

            Ok(())
        })?;

    println!("All test cases passed!");
    Ok(())
}
